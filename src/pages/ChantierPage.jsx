      ...
      <div style={{ maxWidth: "900px", width: "100%" }}>
        <div style={{ background: "white", borderRadius: 6, border: "1px solid #e5e7eb", marginBottom: "0.75rem", overflow: "hidden" }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            <tbody>
              {actifs.length === 0 ? (
                <tr><td style={{ padding: 8, textAlign: "center", color: "#9ca3af" }}>Aucun chantier</td></tr>
                ...
                <tr key={chantier.id} style={{ borderBottom: "1px solid #d1d5db", background: idx % 2 === 0 ? "white" : "#f3f4f6" }}>
                  ...
                  <td style={{ padding: 8, textAlign: "center" }}><button onClick={() => setEditingChantier(chantier)} style={{ padding: "2px 6px", border: "1px solid #d1d5db", background: "white", borderRadius: 3, fontSize: 10, cursor: "pointer" }}>✏️</button></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {archived.length > 0 && (
          <div style={{ background: "white", borderRadius: 6, border: "1px solid #e5e7eb", overflow: "hidden" }}>
            <div onClick={() => setIsArchivedExpanded(!isArchivedExpanded)} style={{ background: "#6b7280", color: "white", padding: "0.75rem 1rem", fontWeight: 600, fontSize: 13, cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center", userSelect: "none", transition: "background 0.2s" }} onMouseEnter={(e) => e.currentTarget.style.background = "#4b5563"} onMouseLeave={(e) => e.currentTarget.style.background = "#6b7280"}>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                <tbody>
                  {archived.map((chantier, idx) => (
                    <tr key={chantier.id} style={{ borderBottom: "1px solid #d1d5db", background: idx % 2 === 0 ? "white" : "#f3f4f6" }}>
                      <td style={{ padding: 8, color: "#374151", fontWeight: 500, fontSize: 11 }}>{chantier.nom}</td>
                      <td style={{ padding: 8, color: "#374151", fontSize: 10 }}>{formatDate(chantier.dateDebut)} → {formatDate(chantier.dateFin)}</td>
                      <td style={{ padding: 8, textAlign: "center" }}><button onClick={() => setEditingChantier(chantier)} style={{ padding: "2px 6px", border: "1px solid #d1d5db", background: "white", borderRadius: 3, fontSize: 10, cursor: "pointer" }}>↻</button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <Modal isOpen={showAddModal} title="Nouveau chantier" onClose={() => setShowAddModal(false)}>
          <FormChantier onSubmit={handleAddChantier} onCancel={() => setShowAddModal(false)} mode="add" />
        </Modal>

        <Modal isOpen={!!editingChantier} title="Modifier le chantier" onClose={() => setEditingChantier(null)}>
          {editingChantier && <FormChantier chantier={editingChantier} onSubmit={handleUpdateChantier} onCancel={() => setEditingChantier(null)} mode="edit" />}
        </Modal>
      </div>
    </div>
  );
