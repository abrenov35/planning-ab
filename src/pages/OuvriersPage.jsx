...
export const OuvriersPage = () => {
  ...
  const table = (items, emptyLabel, actionLabel = "✏️") => <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}><tbody>{items.length === 0 ? <tr><td style={{ padding:8, textAlign:"center", color:"#9ca3af" }}>{emptyLabel}</td></tr> : items.map((ouvrier, idx) => <tr key={ouvrier.id} style={{ borderBottom:"1px solid #d1d5db", background:idx % 2 === 0 ? "white" : "#f3f4f6" }}><td style={{ padding:8, color:"#374151", fontWeight:500 }}>{ouvrier.nom}</td><td style={{ padding:8, textAlign:"center", width:52 }}><button onClick={() => setEditingOuvrier(ouvrier)} style={{ padding:"2px 6px", border:"1px solid #d1d5db", background:"white", borderRadius:3, fontSize:10, cursor:"pointer" }}>{actionLabel}</button></td></tr>)}</tbody></table>;
