...
export const OuvriersPage = () => {
  ...
  const buildReordered = (list, worker, positionApres) => {
    ...
    if (!after) { ordered.unshift(worker); return ordered; }
    const index = ordered.findIndex(o => normalizeWorkerName(o.nom) === after);
    if (index === -1) ordered.push(worker); else ordered.splice(index + 1, 0, worker);
    return ordered;
  };
  ...
  const persistActiveOrder = async (ordered, baseline = ouvriers) => {
    const updates = ordered.flatMap((o, i) => {
      ...
      const ordre = i + 1;
      const changed = !previous || previous.nom !== o.nom || previous.type !== o.type || previous.metier !== o.metier || previous.statut !== o.statut || Number(previous.ordre) !== ordre || !!previous.separateurApres !== !!o.separateurApres || String(previous.couleurCellule || "") !== String(o.couleurCellule || "");
      return changed ? [updateOuvrier(o.id, o.nom, o.type, o.metier, o.statut, ordre, !!o.separateurApres, false, o.couleurCellule || "")] : [];
    });
  ...
  const handleAddOuvrier = async formData => {
    ...
    if (!result.success) { alert("Erreur: " + (result.error || "Impossible d'ajouter l'ouvrier")); return false; }
    try {
      const nouveau = { id: result.id, nom: formData.nom, type: formData.type, metier: formData.metier, statut: "Actif", separateurApres: !!formData.separateurApres, couleurCellule:formData.couleurCellule || "" };
      const ordered = buildReordered(ouvriers, nouveau, formData.positionApres);
      await persistActiveOrder(ordered);
      await loadData(false);
      setShowAddModal(false);
      return true;
    } catch (err) { alert("Ouvrier créé, mais ordre non enregistré : " + err.message); return false; }
  };

  const handleUpdateOuvrier = async formData => {
    try {
      const edited = { ...editingOuvrier, ...formData, couleurCellule:formData.couleurCellule || "", separateurApres: !!formData.separateurApres };
      if (formData.statut !== "Actif") {
        const archivedResult = await updateOuvrier(edited.id, edited.nom, edited.type, edited.metier, edited.statut, "", false, false, edited.couleurCellule);
  ...
  const table = (items, emptyLabel, archivedMode = false) => <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}><tbody>{items.length === 0 ? <tr><td style={{ padding:8, textAlign:"center", color:"#9ca3af" }}>{emptyLabel}</td></tr> : items.map((ouvrier, idx) => <tr key={ouvrier.id} style={{ borderBottom:"1px solid #d1d5db", background:idx % 2 === 0 ? "white" : "#f3f4f6" }}><td style={{ padding:8, color:"#374151", fontWeight:500 }}><span onClick={() => setEditingOuvrier(ouvrier)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setEditingOuvrier(ouvrier); } }} role="button" tabIndex={0} title="Modifier l'ouvrier" style={{ cursor:"pointer" }}>{ouvrier.nom}</span></td>{archivedMode && <td style={{ padding:8, textAlign:"center", width:52 }}><button onClick={() => setEditingOuvrier(ouvrier)} style={{ padding:"2px 6px", border:"1px solid #d1d5db", background:"white", borderRadius:3, fontSize:10, cursor:"pointer" }}>↻</button></td>}</tr>)}</tbody></table>;
