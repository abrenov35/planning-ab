...
export const GanttChart = ({ ouvriers, chantiers, affectations, onAffectationClick, onAddAffectation, onControlsReady }) => {
  ...
  const mobileLandscapeMediaQuery = "(max-width: 1100px) and (pointer: coarse) and (orientation: landscape)";
  ...
  const [isMobileLandscape, setIsMobileLandscape] = useState(() => typeof window !== "undefined" && window.matchMedia(mobileLandscapeMediaQuery).matches);

  React.useEffect(() => {
    if (typeof window === "undefined") return;
    const media = window.matchMedia(mobileMediaQuery);
    const landscapeMedia = window.matchMedia(mobileLandscapeMediaQuery);
    const update = () => {
    ...
    if (media.addEventListener) {
      ...
      landscapeMedia.addListener(update);
    }
    ...
    return () => {
      if (media.removeEventListener) {
        ...
        landscapeMedia.removeEventListener("change", update);
      } else {
        media.removeListener(update);
        landscapeMedia.removeListener(update);
      }
  ...
  const workerColumnWidth=isMobile?(isMobileLandscape?104:92):150;
  const mobileTimelineWidth=isMobileLandscape?1180:1120;
  ...
  const outerPadding=isMobileLandscape?"0.2rem":isMobile?"0.35rem":"1rem";
  const legendStyle=isMobileLandscape
    ? {gap:"0.75rem",padding:"0.35rem 0.45rem",marginBottom:"0.25rem",fontSize:"12px"}
    : {gap:"1rem",padding:"0.75rem 0.5rem",marginBottom:"0.5rem",fontSize:"11px"};

  return <div style={{padding:outerPadding,flex:1,display:"flex",flexDirection:"column",minWidth:0,minHeight:0,height:isMobileLandscape?"100%":undefined,overflow:isMobileLandscape?"hidden":undefined}}>
    <div style={{display:"flex",alignItems:"center",flexWrap:"wrap",background:"rgba(255,255,255,0.5)",borderRadius:"4px",...legendStyle}}>
      {chantiersActifs.map(chantier=><div key={chantier.id} style={{display:"flex",alignItems:"center",gap:"6px"}}><div style={{width:isMobileLandscape?"11px":"10px",height:isMobileLandscape?"11px":"10px",backgroundColor:getChantierColor(chantier.id),borderRadius:"2px",flexShrink:0}}/><span style={{color:"#4b5563",fontWeight:500}}>{chantier.nom}</span></div>)}
    </div>
    <div style={{background:"white",borderRadius:6,border:"1px solid #e5e7eb",display:"flex",flexDirection:"column",flex:1,overflowY:"auto",overflowX:"auto",WebkitOverflowScrolling:"touch",touchAction:"pan-x pan-y",overscrollBehaviorX:"contain",minWidth:0,minHeight:0}}>
      <div style={{display:"flex",height:"60px",flexShrink:0,...rowWidthStyle}}>
        ...
        <div style={{height:"60px",background:"#e5e7eb",borderRight:"1px solid #9ca3af",...timelineFlexStyle}}>
          <div style={{display:"grid",gridTemplateColumns:gridTemplate,height:"22px",borderBottom:"1px solid #cbd5e1",background:"#eef2f7"}}>
            {monthGroups.map((group,index)=><div key={group.key} style={{gridColumn:`${group.start+1} / span ${group.count}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:isMobileLandscape?11:10,fontWeight:800,letterSpacing:"0.04em",color:"#334155",background:monthBandColors[index%monthBandColors.length]}}>{group.label}</div>)}
          </div>
          <div style={{display:"grid",gridTemplateColumns:gridTemplate,height:"38px",background:"#e5e7eb"}}>
            {allDates.map((date,idx)=><div key={idx} style={{padding:"0.45rem 0.55rem",borderRight:getDayRightBorder(idx),textAlign:"center",fontSize:isMobileLandscape?10:9,fontWeight:600,color:"#1f2937",display:"flex",alignItems:"center",justifyContent:"center"}}>{formatShortDate(date)}</div>)}
          </div>
        </div>
      </div>
      ...
      {ouvriersActifs.map((ouvrier,idx)=>{
        ...
        return <div key={ouvrier.id} style={rowWidthStyle}>
          <div style={{display:"flex",height:`${rowHeight}px`,background:rowBackground,...rowWidthStyle}}>
            <div style={{width:workerColumnWidth,padding:isMobile?"0.5rem 0.45rem":"0.5rem 0.75rem",background:rowBackground,borderRight:"1px solid #9ca3af",fontSize:isMobileLandscape?11:10,fontWeight:600,color:"#1f2937",display:"flex",flexDirection:"column",justifyContent:"center",flexShrink:0,...stickyWorkerStyle}}><div>{ouvrier.nom}</div></div>
